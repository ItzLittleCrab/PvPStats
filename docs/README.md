...
<p>
  <a href="../README.md">🏠 返回主页 · Back to Home</a> &nbsp;·&nbsp;
...
| 指令 | 权限 | 描述 |
|------|------|------|
| `/stats` | `crabpvpstats.stats.self` | 查看自己的 PvP 统计数据 |
...
| `crabpvpstats.admin` | `op`（管理员） | 管理权限（预留） |
...
输出文件：`target/crabpvpstatus-1.0.0.jar`（约 13 MB，已包含 sqlite-jdbc）
...
|---------|------------|-------------|
| `/stats` | `crabpvpstats.stats.self` | View your own PvP stats |
...
| `crabpvpstats.admin` | `op` | Admin access (reserved) |
...
Output: `target/crabpvpstatus-1.0.0.jar` (~13 MB, sqlite-jdbc shaded)
